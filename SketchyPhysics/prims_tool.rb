...
def previewLathedObject(view)
    if @solidHeight < 0
        ...
        xform = Geom::Transformation.new(@inputPoints[0].position, @inputPoints[0].normal.reverse)
    else
        xform = Geom::Transformation.new(@inputPoints[0].position, @inputPoints[0].normal)
    end
...
def previewShapeCursor(view)
    if @ip.normal
        pts = sp_points_on_circle(@ip.position, @ip.normal, 3, @numSegments, 0)
        view.draw(GL_LINE_STRIP, pts)

        #~ pts = sp_points_on_circle(@ip.position, @ip.normal, 1.25, @numSegments, 0)
        #~ view.draw(GL_LINE_STRIP, pts)

        #~ xform=Geom::Transformation.new(@ip.position, @ip.normal)
...
def previewSphere(view)
    ...
    if @shapeRadius
        0.step(180,30){ |r|
            center = @inputPoints[0].position
            unless @ctrlDown
                n = @inputPoints[0].normal
                n.length = @shapeRadius
                center = @inputPoints[0].position+n
            end
            rn = @inputPoints[0].normal.transform(Geom::Transformation.rotation([0,0,0], [0,1,0], r.degrees))
            view.draw(GL_LINE_STRIP, sp_points_on_circle(center, rn, @shapeRadius, @numSegments, 0))
            rn = @inputPoints[0].normal.transform(Geom::Transformation.rotation([0,0,0], [1,0,0], r.degrees))
            view.draw(GL_LINE_STRIP,sp_points_on_circle(center, rn, @shapeRadius, @numSegments, 0))
        }
    end
end
...
def previewShape(view)
    ...
    @shapePoints = sp_points_on_circle(center, @inputPoints[0].normal, radius, @numSegments, angleDelta)
    @shapeRadius = radius
...
def drawPreviewShape(view)
    ...
    @bDrawNormal = true
    if @bDrawNormal && @ip.normal != nil
        normal = @ip.normal
        normal.length = 3
        view.draw(GL_LINE_STRIP, [@ip.position, @ip.position+@ip.normal])
    end
...
class Sketchup::InputPoint
    def normal
        if face
            nrml = face.normal.transform(transformation)
        else
            if position[2] == 0.0
                nrml = Geom::Vector3d.new(0,0,1)
            elsif position[1] == 0.0
                nrml = Geom::Vector3d.new(0,1,0)
            elsif position[0] == 0.0
                nrml = Geom::Vector3d.new(1,0,0)
            else
                nrml = Geom::Vector3d.new(0,0,1)
            end
        end
    end
end # class Sketchup::InputPoint
    ...
    case @primType
        ...
        when 'cylinder'
            if @inputPoints.length == 2
                ...
                @centerLine = model.entities.add_cline(@inputPoints[0].position, @inputPoints[0].normal)
            end
        when 'box'
            if @inputPoints.length == 2
                if Sketchup.version.to_i > 6
        ...
        when 'sphere'
            ...
            if @inputPoints.length == 2
                ...
                grp.transform!(Geom::Transformation.new(center, @inputPoints[0].normal))
...
unless file_loaded?(__FILE__)
    ...
    cmd.small_icon = 'images/SketchySolids-Cube.png'
    cmd.large_icon = 'images/SketchySolids-Cube.png'
    cmd.menu_text = cmd.tooltip = 'Create box'
    cmd.status_bar_text = 'Create box'
    toolbar.add_item cmd


    cmd = UI::Command.new('Sphere'){ MSketchyPhysics3.createPrim('sphere') }
    cmd.small_icon = 'images/SketchySolids-Sphere.png'
    cmd.large_icon = 'images/SketchySolids-Sphere.png'
    cmd.menu_text = cmd.tooltip = 'Create sphere'
    cmd.status_bar_text = 'Create sphere'
    toolbar.add_item cmd


    cmd = UI::Command.new('Cylinder'){ MSketchyPhysics3.createPrim('cylinder') }
    cmd.small_icon = 'images/SketchySolids-Cylinder.png'
    cmd.large_icon = 'images/SketchySolids-Cylinder.png'
    cmd.menu_text = cmd.tooltip = 'Create cylinder'
    cmd.status_bar_text = 'Create cylinder'
    toolbar.add_item cmd


    cmd = UI::Command.new('Cone'){ MSketchyPhysics3.createPrim('cone') }
    cmd.small_icon = 'images/SketchySolids-Cone.png'
    cmd.large_icon = 'images/SketchySolids-Cone.png'
    cmd.menu_text = cmd.tooltip = 'Create cone'
    cmd.status_bar_text = 'Create cone'
    toolbar.add_item cmd


    cmd = UI::Command.new('Capsule'){ MSketchyPhysics3.createPrim('capsule') }
    cmd.small_icon = 'images/SketchySolids-Capsule.png'
    cmd.large_icon = 'images/SketchySolids-Capsule.png'
    cmd.menu_text = cmd.tooltip = 'Create capsule or chamfer'
    cmd.status_bar_text = 'Create capsule or chamfer'
    toolbar.add_item cmd
    ...
    #~cmd.status_bar_text = 'Create torus'
    #~toolbar.add_item cmd
    ...
    cmd.menu_text = cmd.tooltip = 'Create a solid floor'
    cmd.status_bar_text = 'Create a solid floor'
    toolbar.add_item cmd

    toolbar.add_separator

    cmd = UI::Command.new('Wheel'){ MSketchyPhysics3.createPrim('wheel') }
    cmd.small_icon = 'images/SketchySolids-Wheel.png'
    cmd.large_icon = 'images/SketchySolids-Wheel.png'
    cmd.menu_text = cmd.tooltip = 'Create wheel. A capsule with a hinge or servo (press CTRL) at center.'
    ...
    cmd.small_icon = 'images/SketchySolids-Door.png'
    cmd.large_icon = 'images/SketchySolids-Door.png'
    cmd.menu_text = cmd.tooltip = 'Create door. A box with a hinge or servo (press CTRL) at the first corner.'
    ...
    cmd.small_icon = 'images/SketchySolids-Lift.png'
    cmd.large_icon = 'images/SketchySolids-Lift.png'
    cmd.menu_text = cmd.tooltip = 'Create lift. A capsule with a built in slider or piston (press CTRL) at center.'
