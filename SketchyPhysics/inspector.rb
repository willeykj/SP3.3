...
class SketchyPhysics
    ...
    def self.addWatermarkText(x, y, text, name = 'watermark', component = 'version_text.skp')
    ...
    def self.checkVersion
        ...
        if version.nil? || version != sp_version
            ...
            if state
                msg = "This model was made with SketchyPhysics #{version}.\n"
                msg << "Your version is #{sp_version}.\n"
                msg << "Model may not work correctly!"
                UI.messagebox(msg, MB_OK, 'Warning!')
...
class PhysicsObjectInspector
    ...
    def showDialog
        ...
        dlg.set_on_close {
            @physicsInspectorDialog = nil
            cmd = 'var tag = document.activeElement.tagName;'
            cmd << 'if (tag.indexOf("BODY") == -1 && tag.indexOf("HTML") == -1) document.activeElement.blur();'
    ...
    def inspectJointProperties(group)
        ...
        %w(min max accel maxAccel strength damp duration Controller onTick breakingForce ConnectedCollide GearConnectedCollide range falloff delay rate eachFrame gearjoint ratio).each { |key|
            ...
            if value.is_a?(Array)
                ...
                tableBody += "<input type='checkbox' onclick='#{onclick}' name='#{fullName}' #{checked}>"+""+'</input>'
    ...
    def inspectGroup(group)
        ...
        str = ''
        #str += "setObjectShape(\""+group.get_attribute('SPOBJ',"shape","default")+"\");"
        count = 0
        ...
        statenames.each{ |sn|
            ...
            onclick = "setSkpAttributeBool(this.name, this.checked)"
            checked = value ? 'checked' : ''
            html = "<input type='checkbox' onblur='#{onclick}' onclick='#{onclick}' name='#{fullName}' #{checked}>"+sn+'</input>'
        ...
        unless isJoint(group)
            ...
            statenames.each { |sn|
                ...
                if sn == 'magnet'
                    ...
                    html += "<input type='checkbox' onClick='#{onclick}' name='#{fullName}' #{checked}>Magnet"+'</input></td>'
                    ...
                    html += "<input type='checkbox' onClick='#{onclick}' name='#{fullName}' #{checked}>Thruster"+'</input>'
                    html += "<td width='25%' align='right' valign='bottom'>Strength</td>"
                    ...
                    density = escapeHTML(group.get_attribute('SPOBJ', 'density', 0.2))
                    strname = group.entityID.to_s + '.SPOBJ.' + 'density'
                    onchange = "setSkpAttribute(this.name, this.value)"
                    html = "<div class='propertyGrid'><table width='100%'><tr>" # <td width='50%'>"
                    html += "<td>"
                    html += "</td><td>Density<input type='text' size='5' onblur='#{onchange}'  onkeypress='validateNumber();' onkeyup='markDirty(this,this.name,this.id);'  name='#{strname}' value=#{density}>"
                    #html + ="ID<input type='text' class='formulaInput' id='materialid' size='12' onblur='#{onchange}' name='#{strname}' value=#{matName}></input>"
                    html += "</td></tr></table></div>"
                elsif sn == 'emitter'
                    html = "<div class='propertyGrid'><table width='100%'><tr><td width='25%'>"
                    html += "<input type='checkbox' onClick='#{onclick}' name='#{fullName}' #{checked}>Emitter"+'</input></td>'
                    onchange = "setSkpAttributeString(this.name,this.id)"
                    strname = group.entityID.to_s + '.SPOBJ.' + 'emitterstrength'
                    strength = escapeHTML(group.get_attribute('SPOBJ', 'emitterstrength', 0.0))
                    html += "<td width='25%' align='right' valign='bottom'>Strength</td>"
                    html += "<td><input type='text' class='formulaInput' id='emitterstrength' style='width:98%' onblur='#{onchange}'  onkeyup='markDirty(this,this.name,this.id);' name='#{strname}' value=#{strength}></input>"
                    html += "</td></tr>"
                    html += "<tr><table width='100%'><tr>"
                    lifetime = escapeHTML(group.get_attribute('SPOBJ', 'lifetime', 0).to_i)
                    strname = group.entityID.to_s + '.SPOBJ.' + 'lifetime'
                    onchange = "setSkpAttribute(this.name, this.value)"
                    html += "<td width='1%' valign='bottom'>Lifetime</td>"
                    html += "<td><input type='text' style='width:98%' onblur='#{onchange}' id='lifetime' onkeypress='validateNumber();' onkeyup='markDirty(this,this.name,this.id);' name='#{strname}' value=#{lifetime}>"
                    html += "</td>"
                    onchange = "setSkpAttributeString(this.name, this.id)"
                    rate = escapeHTML(group.get_attribute('SPOBJ', 'emitterrate', 0))
                    ratestrname = group.entityID.to_s + '.SPOBJ.' + 'emitterrate'
                    html += "<td width='1%' align='right' valign='bottom'>Rate</td><td><input type='text' class='formulaInput' id='emitterrate' style='width:98%' onblur='#{onchange}'  onkeyup='markDirty(this,this.name,this.id);' name='#{ratestrname}' value=#{rate}></input>"
                    html += "</td>"
                    ...
                    html += "<td width='50%'><input type='checkbox' onClick='#{toggleclick}' name='#{fullName}' #{checked}>OnTouch"+'</input></td>'
                    onchange = "setSkpAttributeString(this.name,this.id)"
                    ...
                    html += "<table width='100%'><tr width='100%'><td width='100%'><textarea class='formulaInput' type='text' cols=30 ROWS=5 style='width:100%;display:#{vis}' class='formulaInput' id='ontouch' onblur='#{onchange}' onkeyup='markDirty(this,this.name,this.id);'  name='#{strname}' value=#{ontouch}>#{ontouch}</textarea>"
                    html += "</td></tr></table></div>"
                elsif sn == 'tickable'
                    toggleclick = onclick + ';setDivVisibility(\"ontick\",this.checked)'
                    html = "<div class='propertyGrid'><table width='100%'><tr>"
                    html += "<td width='50%'><input type='checkbox' onClick='#{toggleclick}' name='#{fullName}' #{checked}>OnTick"+'</input></td>'
                    onchange = "setSkpAttributeString(this.name, this.id)"
                    ...
                    vis = (checked == 'checked') ? 'block' : 'none'
                    html += "<table width='100%'><tr width='100%'><td width='100%'><textarea class='formulaInput' type='text' cols=30 ROWS=5 style='width:100%;display:#{vis}' class='formulaInput' id='ontick' onblur='#{onchange}' onkeyup='markDirty(this,this.name,this.id);' name='#{strname}' value=#{ontick}>#{ontick}</textarea>"
                    html += "</td></tr></table></div>"
                elsif sn == 'scripted'
                    toggleclick = onclick + ';setDivVisibility(\"script\",this.checked)'
                    html = "<div class='propertyGrid'><table width='100%'><tr>"
                    html += "<td width='50%'><input type='checkbox' onClick='#{toggleclick}' name='#{fullName}' #{checked}>Scripted"+'</input></td>'
                    onchange = "setSkpAttributeString(this.name,this.id)"
                    strname = group.entityID.to_s + '.SPOBJ.' + 'script'
                    script = escapeHTML(group.get_attribute('SPOBJ', 'script', ''))
                    vis = (checked == 'checked') ? 'block' : 'none'
                    html += "<table width='100%'><tr width='100%'><td width='100%'><textarea class='formulaInput' type='text' WRAP=OFF cols=30 ROWS=10 style='width:100%;display:#{vis}' class='formulaInput' id='script' onblur='#{onchange}' onkeyup='markDirty(this,this.name,this.id);' onkeydown='AllowTabCharacter();' name='#{strname}' value=#{script}>#{script}</textarea>"
                    html += "</td></tr></table></div>"
                end
                str += html
            }
